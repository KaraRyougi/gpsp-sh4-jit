...
u8 bios_rom[1024 * 16] CGBA_HIGH_BSS;

// Up to 128kb, store SRAM, flash ROM, or EEPROM here.
u8 gamepak_backup[1024 * 128] CGBA_HIGH_BSS;
...
static void free_gamepak_mini_rom(void)
{
  if (!gamepak_mini_rom)
    return;
...
static u32 evict_gamepak_page(void)
{
  ...
  do {
    ...
    if (++spins > 32 * 1024) {
      ...
      const char *s = msg;
      while (*s) *(volatile unsigned char *)0xb7000000u = *s++;
...
u32 load_gamepak_from_memory(const u8 *rom, u32 rom_size,
                             ...
                             int force_serial)
{
  ...
  if (gamepak_buffer_count > 0)
  {
    u32 copy_size = raw_size < gamepak_buffer_blocksize ?
                   raw_size : gamepak_buffer_blocksize;
    memcpy(gamepak_buffers[0], gamepak_mini_rom, copy_size);
    if (copy_size < gamepak_buffer_blocksize)
      memset(gamepak_buffers[0] + copy_size, 0xFF,
             gamepak_buffer_blocksize - copy_size);
  }
  ...
  gamepak_header_nonstandard =
      (raw_size <= 0xB2) ||
      (gamepak_mini_rom[3] != 0xEA) ||
      (gamepak_mini_rom[0xB2] != 0x96);

  if (raw_size > 0xAF)
    memcpy(game_code, &gamepak_mini_rom[0xAC], 4);
  ...
  {
    u32 scan_size = raw_size < (1024*1024) ? raw_size : (1024*1024);
    detect_backup_subcircuit(gamepak_mini_rom, scan_size);
  }
...
u32 load_gamepak_from_pages(const u8 * const *pages, u32 rom_size,
                            int force_rtc, int force_rumble,
                            int force_serial)
{
  char game_code[5] = {0,0,0,0,0};
  ...
  if (!pages || rom_size == 0 || rom_size > 0x20000000)
    return (u32)-1;
  if (rom_size & 0x7FFF)
    return (u32)-1;
  ...
  free_gamepak_mini_rom();
  gamepak_mini_materialized = false;
  gamepak_file_blocks = map_blocks;
  gamepak_size = raw_size;
  gamepak_mirror_1m = false;
  ...
  if (raw_size > 0xAF)
    memcpy(game_code, &header[0xAC], 4);
