 ...
 * Text scripts, one event per line: "<frame> <mask>" (decimal frame number,
 * hex GBA button mask in KEYINPUT bit order: A B Sel Start R L U D R-shldr
 * L-shldr). An event sets the held-button state FROM its frame until the
 * next event. Purpose: deep-gameplay testing — synthetic one/two-button
 * harnesses cannot navigate real menus or battles, so recordings of REAL
 * play (captured on the calculator or the interactive emulator) become
 * replayable regression assets for the headless harness.
 *
 * Recording is always on (append-on-change into a small RAM ring); the
 * recording is written to \\fls0\\<BASE>.INP as a side effect of SAVING A
 * STATE, pairing the input path with the state it produced. The headless
 * harness loads <ROM dir>/GAME.INP (same <BASE>.INP naming) at boot and
 * replays it verbatim. */
#define CGBA_INP_MAX_EVENTS 6144u
static struct cgba_inp_event { u32 frame; u16 mask; } cgba_inp_ev[CGBA_INP_MAX_EVENTS];
static unsigned cgba_inp_count;
...
static void cgba_inp_path(uint16_t *path)
{
	...
	for (b = 0; base[b]; b++)
		path[i++] = (uint16_t)base[b];
	path[i++] = '.'; path[i++] = 'I'; path[i++] = 'N'; path[i++] = 'P';
	path[i] = 0;
}
...
void cgba_gpsp_input_record_frame(unsigned frame, unsigned mask)
{
	...
	if (cgba_inp_count > 0 && (u16)mask == cgba_inp_last_mask)
		return;
	if (cgba_inp_count >= CGBA_INP_MAX_EVENTS)
		return;                          /* ring full: keep the prefix */
	cgba_inp_ev[cgba_inp_count].frame = frame;
	cgba_inp_ev[cgba_inp_count].mask = (u16)mask;
	cgba_inp_count++;
	cgba_inp_last_mask = (u16)mask;
}

/* Render + write the recording; returns 1 on success, 0 when empty/failed.
 * Borrows the savestate staging buffer (and therefore flushes the ROM
 * translation cache afterwards, exactly like state_save). */
/* Render the recording as text lines into buf; returns byte length (0 when
 * empty). Exposed so the headless harness can write it via its own storage
 * path (the generic BFile writes are not available under the emulator). */
unsigned cgba_gpsp_input_record_render(unsigned char *buf, unsigned cap)
{
	...
	if (cgba_inp_count == 0)
		return 0;
	for (i = 0; i < cgba_inp_count; i++) {
		char line[16];
		unsigned k = 0, v = cgba_inp_ev[i].frame;
		char tmp[10];
		unsigned t = 0;
		do { tmp[t++] = (char)('0' + v % 10); v /= 10; } while (v);
		while (t) line[k++] = tmp[--t];
		line[k++] = ' ';
		{
			static const char h[] = "0123456789ABCDEF";
			u16 m = cgba_inp_ev[i].mask;
			line[k++] = h[(m >> 8) & 0xF];
			line[k++] = h[(m >> 4) & 0xF];
			line[k++] = h[m & 0xF];
		}
...
void cgba_gpsp_input_path(uint16_t *path)
{
	cgba_inp_path(path);
}
...
int cgba_gpsp_input_record_save(void)
{
	...
	uint16_t path[24];
	unsigned n = cgba_gpsp_input_record_render(buf, 131072u);
	int ok;

	if (n == 0)
		return 0;
	cgba_inp_path(path);
	ok = fxcg100_storage_write_blob(path, buf, n);
#ifdef CGBA_DYNAREC
...
/* Load <BASE>.INP into the event table; returns the event count (0 = none).
 * Also borrows the staging buffer (call before/around cache-sensitive work;
 * flushes the ROM cache afterwards under the dynarec). */
int cgba_gpsp_input_script_load(void)
{
	...
	if (fsz <= 0 || fsz > 131072)
		return 0;
	if (!fxcg100_storage_read_blob(path, buf, (unsigned)fsz))
		return 0;
	n = (unsigned)fsz;
	while (i < n && cgba_inp_count < CGBA_INP_MAX_EVENTS) {
		u32 frame = 0, mask = 0;
		int have = 0;
		while (i < n && buf[i] >= '0' && buf[i] <= '9') {
			frame = frame * 10 + (u32)(buf[i++] - '0');
			have = 1;
		}
		...
		if (have) {
			cgba_inp_ev[cgba_inp_count].frame = frame;
			cgba_inp_ev[cgba_inp_count].mask = (u16)(mask & 0x3FF);
			cgba_inp_count++;
		}
	}
#ifdef CGBA_DYNAREC
	flush_translation_cache_rom();
#endif
	cgba_inp_script_loaded = (cgba_inp_count > 0);
	return (int)cgba_inp_count;
}

int cgba_gpsp_input_script_active(void)
{
	return cgba_inp_script_loaded;
}

/* Held mask for `frame` (events are in ascending frame order; the cursor
 * only moves forward — harness frames are monotonic). */
unsigned cgba_gpsp_input_script_mask(unsigned frame)
{
	while (cgba_inp_cursor < cgba_inp_count &&
	       cgba_inp_ev[cgba_inp_cursor].frame <= frame)
		cgba_inp_cursor++;
	if (cgba_inp_cursor == 0)
		return 0;
	return cgba_inp_ev[cgba_inp_cursor - 1].mask;
}
