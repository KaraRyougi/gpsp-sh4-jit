...
/*
 * Native SH-4A emission for Thumb block loads (POP/LDMIA).
 *
 * The hardware profile showed Thumb block helpers dominating runtime. The hot
 * safe subset is stack/register-list loads from plain RAM; stores remain on the
 * C helper so DMA/IRQ alerts and SMC invalidation stay centralized.
 */
  ...
  int cycle_count)
{
  ...
  u32 base_reg, load_pc = 0, count = 0, i;
  ...
  if (hi == 0xBC || hi == 0xBD) {             /* POP {rlist[,pc]} */
    base_reg = 13;                            /* SP */
    load_pc = (hi == 0xBD);
  } else if (hi >= 0xC8 && hi <= 0xCF) {      /* LDMIA Rb!,{rlist} */
    base_reg = (opcode >> 8) & 7;
  } else {
    return 0;                                 /* PUSH/STM stay helper-owned */
  }
  ...
  /* R1 = original base, R5 = first transfer address. */
  { sh4_codegen cg = sh4g_open(tp);
    sh4_emit_load_greg(&cg, base_reg, SH4_REG_T0);
    sh4_emit_mov_reg(&cg, SH4_REG_T0, SH4_REG_ARG1);
    sh4g_close(tp, &cg); }

  /* Native SH4 word loads require alignment. The C helper handles rotate cases. */
  { sh4_codegen cg = sh4g_open(tp);
