...
- **Yoshi A/LEFT soak**: 600 frames from `SUPERM0.SVS` copied in as
  ...
  together in the first 1 MiB GamePak-cache block, preserving both JIT caches
  and their heat state. State loads still flush translated code after replacing
  guest memory. The patched default-96 run completed all
